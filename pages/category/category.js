...
document.addEventListener("DOMContentLoaded", () => {
  ...
  menus.forEach((menu) =>
    menu.addEventListener("click", (event) => getBooksCategory(event))
  );
});
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (category) {
    console.log("선택된 카테고리:", category); //디버깅용 콘솔창
    setCategoryAndFetch(category);
  }
});
...
const getBooks = async () => {
  ...
  const url =
    `${API_PROXY_URL}?apiType=ItemList&QueryType=ItemNewAll&MaxResults=${pageSize}&start=${page}&SearchTarget=Book&output=JS&Version=20131101&Cover=Big` +
...
const paginationRender = () => {
  ...
  const totalPages = Math.ceil(
    Math.min(totalResults, maxResultsLimit) / pageSize
  ); // 최대 200개 제한 적용
  ...
  for (let i = firstPage; i <= lastPage; i++) {
    paginationHTML += `<li class="category-page-item ${
      i === page ? "active" : ""
    }" onclick="moveToPage(${i})"><a class="category-page-link"href="#top">${i}</a></li>`;
  }
  ...
  if (page < totalPages) {
    paginationHTML += `<li class="category-page-item" onclick="moveToPage(${
      page + 1
    })"><a class="category-page-link"href="#top">›</a></li><li class="category-page-item" onclick="moveToPage(${totalPages})"><a class="category-page-link" href="#top">»</a></li>`;
  }
